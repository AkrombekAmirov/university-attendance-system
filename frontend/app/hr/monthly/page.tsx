...
import { fetchHrUnitMonthlyDetailed } from "@/lib/api";
import { Button } from "@/components/ui/button";
import { ArrowLeft, CalendarDays } from "lucide-react";

const weekdaysUz = ["Yak", "Dush", "Sesh", "Chor", "Pay", "Jum", "Shan"];

// 1. Asosiy mantiq va dizaynni alohida komponentga oldik
function HrMonthlyAttendanceContent() {
    ...
    useEffect(() => {
        ...
        (async () => {
            setLoading(true);
            try {
                const data = await fetchHrUnitMonthlyDetailed(unitId, year, month);
                setRows(data || []);
            } catch (err) {
                console.error("Failed to load HR monthly data:", err);
                setRows([]);
            } finally {
                setLoading(false);
            }
    ...
    function getCellClasses(rec: any, jsDay: number, columnIndex: number) {
        ...
        if (!rec || rec.is_absent) {
            return `${base} ${columnBg} bg-rose-300 text-rose-900 cursor-default`;
        }

        // ⏰ KECHIKKAN — OCH QIZIL (soat 10:00 dan keyin)
        const late = rec.first_entry && dayjs(rec.first_entry).isAfter(dayjs(rec.date + " 10:00"));
        if (late) {
            return `${base} ${columnBg} bg-rose-200 text-rose-800 cursor-default`;
        }

        // ✅ VAQTIDA KELGAN — YASHIL
        return `${base} ${columnBg} bg-emerald-200/70 text-emerald-800 cursor-default`;
    }
    ...
    return (
        ...
        >
            ...
            >
                {loading ? (
                    <div className="text-center py-10 text-gray-500 animate-pulse">Yuklanmoqda...</div>
                ) : (
                        ...
                        <tr className="bg-emerald-50 text-emerald-900">
                            ...
                            {dayNumbers.map((d, idx) => {
                                ...
                                return (
                                    <th
                                        key={d}
                                        className={`border border-emerald-200 px-2 py-2 font-bold cursor-default transition-colors ${bgClass}`}
                                    >
                                        {d}
                                    </th>
                                );
...
// 2. Asosiy Export funksiyasi (Suspense bilan o'ralgan holati)
export default function HrMonthlyAttendancePage() {
    return (
        <Suspense fallback={
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-50 via-white to-emerald-100">
                <div className="text-center py-10 text-emerald-600 animate-pulse font-medium text-lg">
                    Sahifa yuklanmoqda...
                </div>
            </div>
        }>
            <HrMonthlyAttendanceContent />
        </Suspense>
    );
}